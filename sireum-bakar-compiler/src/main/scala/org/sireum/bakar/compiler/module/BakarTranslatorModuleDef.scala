...
class BakarTranslatorModuleDef(val job: PipelineJob, info: PipelineJobModuleInfo) extends BakarTranslatorModule {
  ...
  trait VContext {
    ...
    def peekLoopLabel = 
      URIS.addTypeUri(NameExp(URIS.addResourceUri(loopLabelStack.top, URIS.DUMMY_URI)), URIS.DUMMY_URI)      
    ...
    def genTempVar(typeUri: ResourceUri) = {
      ...
      val nts = PilarNodeFactory.buildNamedTypeSpec(typeDecl.id, typeUri)
      val lvd = PilarNodeFactory.buildLocalVar(name, uri, nts)
      localsPush(lvd)

      val ret = URIS.addTypeUri(NameExp(URIS.addResourceUri(NameUser(name), uri)), typeUri)
        ...
        case LoopParameterSpecificationEx(sloc, names, hasRev, range, checks) =>
          ...
          range.getDiscreteSubtypeDefinition match {
            case DiscreteSimpleExpressionRangeAsSubtypeDefinitionEx(sloc2, lbound, hbound, check) =>
              ...
              val markURI = td.uri
              val nu = addResourceUri(addProperty(URIS.REF_URI, td.uri, NameUser(td.id)), td.uri)
    ...
    def handleConstant(v: => BVisitor, o: Base): ISeq[ConstElement] = {
      def make(names: DefiningNameList, typeName: ResourceUri, typeUri: String, initExp: ExpressionClass = null) = {
        ...
        val castExp = CastExp(PilarNodeFactory.buildNamedTypeSpec(typeName, typeUri), ie)
    ...
    def handleVariableDeclaration(v: => BVisitor, o: Base): MList[PilarAstNode] = {
      o match {
        case VariableDeclarationEx(sloc, names, hasAliased, objDecView, initExpr, aspectSpec, checks) =>
          ...
          val typeName = odv.name.name
          val typeUri = odv.name.uri
          val typeSpec = PilarNodeFactory.buildNamedTypeSpec(typeName, typeUri)
          ...
          names.getDefiningNames.foreach {
            case di: DefiningIdentifier =>
              ...
              val vd = if (processingPackage)
                PilarNodeFactory.buildGlobalVar(cdefName, cdefUri, contextStack.top.uri, typeSpec)
              else
                PilarNodeFactory.buildLocalVar(cdefName, cdefUri, typeSpec)
    ...
    def introduceAnonymousType(lowBound: Exp, highBound: Exp,
      ...
      val pilarTypeDec = TypeAliasDecl(
        ...
        PilarNodeFactory.buildNamedTypeSpec(parentTypeName, parentTypeUri))
    ...
    def makeTypeExp(typeName: NameExp, typUri: String) =
      URIS.addTypeUri(TypeExp(PilarNodeFactory.buildNamedTypeSpec(typeName.name.name, typUri)), typUri)
    ...
    def handleTypeDeclaration(o: Base, v: => BVisitor): ISeq[PackageElement] = {
      ...
      val typeDecls = o match {
        ...
        case SubtypeDeclarationEx(sloc, names, typeDeclView, aspectSpecs, checks) =>
          ...
          val pilarTypeDec = TypeAliasDecl(
            addResourceUri(NameDefinition(tname), turi), ivectorEmpty,
            PilarNodeFactory.buildNamedTypeSpec(sname, suri))
  ...
  def packageH(ctx: VContext, v: => BVisitor): VisitorFunction = {
    ...
    def handlePackage(o: Base, sloc: SourceLocation, names: DefiningNameList,
      ...
      if (scope.initBlock.isDefined) {
        ...
        val p = PilarNodeFactory.buildProcedureDecl(name, uri, pname.uri, 
  ...
  def methodH(ctx: VContext, v: => BVisitor): VisitorFunction = {
    def handleMethod(o: Base, sloc: SourceLocation, names: DefiningNameList,
        ...
        case ParameterSpecificationEx(sloc3, pnames, _hasAliased, _hasNullEx,
          ...
          pnames.getDefiningNames.foreach {
            case pname: DefiningIdentifier =>
              ...
              val typeSpec = PilarNodeFactory.buildNamedTypeSpec(odv.name.name, typUri)
              val pd = PilarNodeFactory.buildParamDecl(pname.getDefName, defUri, typeSpec) 
      ...
      val pd = PilarNodeFactory.buildProcedureDecl(methodDefName, methodDefUri,
                      ...
                      def helper(e: Any): ISet[String] = {
                        ...
                        e match {
                          case FunctionCallEx(sloc2, prefix, params, _, _, _, _) =>
                            ...
                            ret ++= helper(x)
                          case PositionalArrayAggregateEx(sloc4, arrassoc, typ, checks) =>
                          ...
                          case IdentifierEx(_, refName, refUri, _, checks) =>
                            ret += ctx.rewriteUri(refUri)
                        }
                        ret
                      }
  ...
  def statementH(ctx: VContext, v: => BVisitor): VisitorFunction = {
    ...
    case ForLoopStatementEx(sloc, labelnames, statementIdentifier,
      ...
      val typeName = markNE.name.name
      val typeUri = markNE.name.uri
      val typeSpec = PilarNodeFactory.buildNamedTypeSpec(typeName, typeUri)
      val lvd = PilarNodeFactory.buildLocalVar(iterND, typeSpec)
      ctx.localsPush(lvd)
  ...
  def aggregateH(ctx: VContext, v: => BVisitor): VisitorFunction = {
    case NamedArrayAggregateEx(sloc, arrayAssocs, typ, checks) =>
      ...
      false
    case PositionalArrayAggregateEx(sloc, arrayAssocs, typ, checks) =>
  ...
  def attributeH(ctx: VContext, v: => BVisitor): VisitorFunction = {
...

    {
      case FirstAttributeEx(sloc, prefix, attributeId, attributeExp, typUri, checks) =>
        ...
        false
      case ImageAttributeEx(sloc, prefix, id, typ, checks) =>
      ...
      case ImplementationDefinedAttributeEx(sloc, prefix, attId, attExp, typeUri, checks) =>
        ...
        false
      case LastAttributeEx(sloc, prefix, attributeId, attributeExp, typ, checks) =>
        assert(attributeExp.getExpressions.isEmpty)
      ...
      case RangeAttributeEx(sloc, prefix, attId, attExp, typ, checks) =>
        ...
        false
      case SuccAttributeEx(sloc, prefix, attrId, typ, checks) =>
        ctx.pushResult(h(prefix, Attribute.ATTRIBUTE_UIF_SUCC), sloc)
        false
      case UnknownAttributeEx(sloc, prefix, id, designatorExps, typ, checks) =>
        v(prefix)
  ...
  def expressionH(ctx: VContext, v: => BVisitor): VisitorFunction = {
    ...
    def handleQuantifiedExp(isUniversal: Boolean, iter: Base, pred: Base, typ: String, checks: String) = {
      ...
      val nts = PilarNodeFactory.buildNamedTypeSpec(markNE.name.name, typUri)
      val pd = PilarNodeFactory.buildParamDecl(iterND, nts)
    ...
    }
    {
      ...
      case QualifiedExpressionEx(sloc, mark, exp, typ, checks) =>
        ...
        val nts = PilarNodeFactory.buildNamedTypeSpec(nu.name, trefUri)
      ...
      case TypeConversionEx(sloc, mark, exp, typ, checks) =>
        ...
        val nts = ctx.addSourceLoc(PilarNodeFactory.buildNamedTypeSpec(refName, refUri), sloc2)
