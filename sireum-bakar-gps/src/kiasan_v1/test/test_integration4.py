...
class TestIntegration4(unittest.TestCase):
    ...
    def test_proj4_all_methods(self):
        kiasan_run_cmd = sireum_v1.get_run_kiasan_command(self.sireum_path, "multicall", self.project_path, self.output_path, False)
        methods = ["main", "logic", "service", "service_logic", "repository"]
        for method in methods[:-1]:
            subprocess.call(kiasan_run_cmd + [method])
        kiasan_run_cmd_with_report = sireum_v1.get_run_kiasan_command(self.sireum_path, "multicall", self.project_path, self.output_path, True)
    ...
    def test_proj4_main(self):
        kiasan_run_cmd = sireum_v1.get_run_kiasan_command(self.sireum_path, "multicall", self.project_path, self.output_path, True)
        methods = ["main"]
