...
class TestIntegration6(unittest.TestCase):
    ...
    def test_proj6_all_methods(self):
        kiasan_run_cmd = sireum_v1.get_run_kiasan_command(self.sireum_path, "RefExample", self.project_path, self.output_path, False)
        methods = ["CreateArray", "ChangeArray", "ChangeGlobalArray"]
        for method in methods[:-1]:
            subprocess.call(kiasan_run_cmd + [method])
        kiasan_run_cmd_with_report = sireum_v1.get_run_kiasan_command(self.sireum_path, "RefExample", self.project_path, self.output_path, True)
    ...
    def test_proj6_delete(self):
        kiasan_run_cmd = sireum_v1.get_run_kiasan_command(self.sireum_path, "RefExample", self.project_path, self.output_path, True)
