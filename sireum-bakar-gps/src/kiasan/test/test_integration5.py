...
class TestIntegration5(unittest.TestCase):
    ...
    def test_proj5_all_methods(self):
        kiasan_run_cmd = sireum.get_run_kiasan_command(self.sireum_path, "ArraySet", self.project_path, self.output_path, False)
        methods = ["Get_Value", "Add", "Delete", "Init"]
        for method in methods[:-1]:
            subprocess.call(kiasan_run_cmd + [method])
        kiasan_run_cmd_with_report = sireum.get_run_kiasan_command(self.sireum_path, "ArraySet", self.project_path, self.output_path, True)
