package body Sort is
  ...
  procedure Swap_Elements(T : in out Array_Type; I, J : in Index_Type) with
    Depends => (T => (T, I, J)),
    Post => (T = T'Old'Update(I => T'Old(J), J => T'Old(I)) and
             Perm(T, T'Old, Index_Type'First, Index_Type'Last))
    ...
    begin 
     for I in reverse Index_Type range Array_Type'Range loop
       for J in Index_Type range Array_Type'First + 1 .. I loop
        ...
        pragma Assert ((for all K in Index_Type range Array_Type'First .. J - 1 =>  
                          (Table(K) <= Table(J)))
           and Perm(Table, Table'Loop_Entry, Index_Type'First, Index_Type'Last));
       ...
       pragma Assert (Ordered(Table, I, Index_Type'Last)
  ...
  begin 
    loop exit when Bound = 0;
      ...
      while J <= Bound - 1 loop
        ...
        pragma Assert((for all K in Index_Type range Index_Type'First .. J - 1 => 
                         (Table(K) <= Table(J))) and
          Partitioned(Table, Index_Type'First, T, J) and 
          Perm(Table, Table'Loop_Entry, Index_Type'First, Index_Type'Last));
        ...
        pragma Assert (Ordered(Table, Bound, Index_Type'Last) and
  ...
  begin 
    for P in Index_Type range Index_Type'First + 1 .. Index_Type'Last loop
      ...
      while (J >= Index_Type'First + 1 and then Table(J - 1) > Temp) loop
        ...
        pragma Assert (
          Temp = Table'Loop_Entry(P) and Index_Type'First <= J and J < P and
          (for all K in Index_Type range J + 1 .. P => (Table(K) > Temp)) and 
          Ordered(Table, Index_Type'First, P) and
          Perm(Table'Update(J => Temp), Table'Loop_Entry, Index_Type'First, P));
      ...
      Table(J) := Temp;
        ...
        pragma Assert (
          Index_Type'First + 1 <= P and P <= Index_Type'Last and
          Ordered(Table, Index_Type'First, P) and
          Perm(Table, Table'Loop_Entry, Index_Type'First, P) and 
